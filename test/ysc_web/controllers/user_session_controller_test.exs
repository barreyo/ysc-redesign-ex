defmodule YscWeb.UserSessionControllerTest do
  ...
  describe "POST /users/log-in" do
    test "redirects to account setup for unverified email users", %{conn: conn, user: user} do
      conn =
    ...
    test "logs the user in with remember me and verified email", %{conn: conn, user: user} do
      # Mark email as verified
      {:ok, _} = Ysc.Accounts.mark_email_verified(user)
    ...
    test "logs the user in with return to and verified email", %{conn: conn, user: user} do
      # Mark email as verified
      {:ok, _} = Ysc.Accounts.mark_email_verified(user)
    ...
    test "login following registration redirects to account setup", %{conn: conn, user: user} do
      conn =
        post(conn, ~p"/users/log-in", %{
          "_action" => "registered",
          "user" => %{
    ...
    test "login following password update with verified email", %{conn: conn, user: user} do
  ...
  describe "GET /users/log-in/auto" do
    test "auto-logs in user with valid token and redirects to pending review", %{conn: conn} do
    ...
    test "handles expired tokens", %{conn: conn, user: user} do
      ...
      expired_time = DateTime.add(DateTime.utc_now(), -61, :day) |> DateTime.truncate(:second)
