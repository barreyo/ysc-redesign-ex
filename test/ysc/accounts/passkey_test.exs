defmodule Ysc.Accounts.PasskeyTest do
  ...
  describe "passkey management" do
    ...
    test "create_user_passkey/2 creates a passkey with valid data", %{user: user} do
    ...
    test "get_user_passkey_by_external_id/1 finds passkey by credential ID", %{user: user} do
      credential_id = :crypto.strong_rand_bytes(16)
    ...
    test "update_passkey_sign_count/2 updates sign_count and last_used_at", %{user: user} do
      credential_id = :crypto.strong_rand_bytes(16)
      ...
      new_sign_count = 5
      assert {:ok, updated_passkey} = Accounts.update_passkey_sign_count(passkey, new_sign_count)
    ...
    test "update_passkey_sign_count/2 handles sign_count decrease (replay attack)", %{user: user} do
      credential_id = :crypto.strong_rand_bytes(16)
      ...
      assert {:ok, updated_passkey} = Accounts.update_passkey_sign_count(passkey, 5)
  ...
  describe "should_show_passkey_prompt?/1" do
    ...
    test "returns true when user has no passkeys and never dismissed", %{user: user} do
    ...
    test "returns false when user dismissed less than 30 days ago", %{user: user} do
      dismissed_at = DateTime.add(DateTime.utc_now(), -15, :day)

      updated_user =
        user
        |> User.update_user_changeset(%{passkey_prompt_dismissed_at: dismissed_at})
        |> Ysc.Repo.update!()
  ...
  describe "dismiss_passkey_prompt/1" do
    ...
    test "sets passkey_prompt_dismissed_at to current time", %{user: user} do
      ...
      assert DateTime.diff(DateTime.utc_now(), updated_user.passkey_prompt_dismissed_at, :second) <
               5
    end
    ...
    test "updates existing dismissal timestamp", %{user: user} do
      ...
      updated_user =
        user
        |> User.update_user_changeset(%{passkey_prompt_dismissed_at: old_dismissed_at})
        |> Ysc.Repo.update!()
        |> Ysc.Repo.reload!()

      assert {:ok, newly_updated_user} = Accounts.dismiss_passkey_prompt(updated_user)
      assert newly_updated_user.passkey_prompt_dismissed_at != old_dismissed_at

      assert DateTime.compare(newly_updated_user.passkey_prompt_dismissed_at, old_dismissed_at) ==
               :gt
    end
  end
end
