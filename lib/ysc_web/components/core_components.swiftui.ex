defmodule YscWeb.CoreComponents.SwiftUI do
  @moduledoc """
  ...
  of the `YscWeb.CoreComponents` functions you should expect identical functionality for similarly named
  ...
  @doc """
  Renders a standard "Back" button for SwiftUI.

  This is intentionally generic: you control behavior by passing `phx-click`
  (or any other button attributes) via `{@rest}`.
  """
  @doc type: :component
  ...
  @doc """
  Renders a rounded "card" container using a `RoundedRectangle` background.

  This avoids relying on `cornerRadius` / modifier ordering, which can be fragile
  in LiveView Native style parsing.
  """
  @doc type: :component
  attr :corner_radius, :integer, default: 22
  attr :fill, :string, default: ".white"
  attr :shadow_opacity, :float, default: 0.10
  attr :shadow_radius, :integer, default: 10
  attr :shadow_x, :integer, default: 0
  attr :shadow_y, :integer, default: 6
  attr :padding, :integer, default: 22
  attr :rest, :global
  slot :inner_block, required: true
  ...
  @doc """
  Renders the YSC logo.

  By default this references a web-served asset at `/images/ysc_logo.png`.
  """
  @doc type: :component
  ...
  def logo(assigns) do
    assigns =
      ...
      |> assign(:class, logo_class(assigns.size))
  ...
  def logo_class(size) when size <= 120, do: "ysc-logo-120"
  def logo_class(size) when size <= 160, do: "ysc-logo-160"
  def logo_class(_size), do: "ysc-logo-200"

  def absolutize_url(nil), do: absolutize_url("/images/ysc_logo.png")
  def absolutize_url("http://" <> _ = url), do: url
  def absolutize_url("https://" <> _ = url), do: url
