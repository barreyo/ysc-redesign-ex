defmodule YscWeb.Emails.ExpenseReportTreasurerNotification do
  ...
  def prepare_email_data(expense_report) do
    ...
    user_info = %{
      name:
        "#{expense_report.user.first_name || ""} #{expense_report.user.last_name || ""}"
        |> String.trim(),
      email: expense_report.user.email
    }
    ...
    # Get address info if present
    address_info =
      if expense_report.address do
        %{
          address: expense_report.address.address,
          city: expense_report.address.city,
          region: expense_report.address.region,
          postal_code: expense_report.address.postal_code,
          country: expense_report.address.country
        }
      else
        nil
      end
