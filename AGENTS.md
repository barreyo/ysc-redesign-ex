...
- When dealing with money we store money in the database as Dollars and use the `ex_money_sql` library where the types for migrations is: `:money_with_currency` and the type in the model code: `Money.Ecto.Composite.Type`. For example `Money.new(:USD, 100)` which will create a USD object for `$100.00`.
...
- **Never** inline CSS. Always use `app.css` if you need to add CSS. Use Tailwind classes as much as possible.
